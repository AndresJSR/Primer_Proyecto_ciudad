export class ApiService {
    ...
    static mockResponse(endpoint, options = {}) {
        ...
        if (endpoint === "/calculate-route") {
            ...
            try {
                ...
                if (!route.length) {
                    return { route: [], message: "No hay ruta disponible entre estos edificios." };
                }
    ...
    static computeMockRoute({ map, origin, destination } = {}) {
        ...
        const destinationSet = new Set(destinationRoads.map((cell) => key(cell.x, cell.y)));
        ...
        while (queue.length > 0) {
            ...
            for (const [dx, dy] of dirs) {
                ...
                if (!inBounds(nx, ny) || Number(map[ny][nx]) !== 1 || visited.has(nextKey)) {
                    continue;
                }
